...
const ChatSelector = ({
  ...
  const ChatSection = ({
    ...
    <div className="mb-4">
      <button
        ...
        className="w-full flex items-center justify-between p-4 hover:bg-base-300 rounded-xl transition-colors group"
      >
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg bg-gradient-to-br ${getSectionColor(color)} ${getTextColor(color)}`}>
            {icon}
          </div>
          <div className="text-left">
            <span className="font-semibold text-base-content block">{title}</span>
  ...
  const renderWorkspaceItem = (workspace) => {
    ...
    return (
      <div
        key={roomId}
        className={`p-3 mx-2 rounded-xl cursor-pointer transition-all duration-200 group ${isActive
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-primary-content font-semibold text-lg flex-shrink-0 ${isActive
          ...
          <div className="flex-1 min-w-0">
            <h4 className={`font-semibold truncate ${isActive ? 'text-primary' : 'text-base-content'
              }`}>
              {workspace.name}
            </h4>
            <p className={`text-sm truncate ${isActive ? 'text-primary/80' : 'text-base-content/60'
              }`}>
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h4 className={`font-semibold truncate ${isActive ? 'text-success' : 'text-base-content'
              }`}>
              {document.title}
            </h4>
            <p className={`text-sm truncate ${isActive ? 'text-success/80' : 'text-base-content/60'
              }`}>
  ...
  const renderFriendItem = (friend) => {
    ...
    return (
      <div
        key={roomId}
        className={`p-3 mx-2 rounded-xl transition-all duration-200 group ${canChat ? 'cursor-pointer' : 'cursor-not-allowed opacity-70'
          } ${isActive
            ? 'bg-gradient-to-r from-primary/20 to-secondary/20 border-2 border-primary/30 shadow-lg'
            : 'hover:bg-base-300 border-2 border-transparent hover:border-base-300'
          }`}
      ...
      >
        <div className="flex items-center gap-3">
          <div className="relative flex-shrink-0">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-primary-content font-semibold text-lg ${isActive
                ? 'bg-gradient-to-br from-primary to-secondary shadow-lg'
            ...
            {isOnline && canChat && (
              <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-success border-2 border-base-100 rounded-full" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              ...
              {isOnline && canChat && (
                <span className="text-xs text-success font-medium flex-shrink-0">Online</span>
              )}
            </div>
          ...
          {status === 'received' && (
            <div className="flex gap-1 flex-shrink-0 ml-2">
              <button
                ...
                className="p-1.5 bg-success text-success-content rounded-lg hover:bg-success/90 transition-colors"
              ...
              <button
                ...
                className="p-1.5 bg-error text-error-content rounded-lg hover:bg-error/90 transition-colors"
  ...
  const renderSearchResultItem = (member) => {
    ...
    return (
      <div className="p-3 rounded-xl border border-base-300 mb-2 bg-base-100 hover:shadow-sm transition-shadow">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative flex-shrink-0">
              <div className="w-10 h-10 bg-gradient-to-br from-primary to-secondary rounded-xl flex items-center justify-center text-primary-content font-semibold">
                {member.avatar?.url ? (
                  <img
          ...
          <div className="flex-shrink-0 ml-2">
            {isConnected ? (
              <span className="px-2 py-1 text-xs bg-success text-success-content rounded-lg whitespace-nowrap">
                Friends
              </span>
            ) : requestReceived ? (
              ...
              <span className="px-2 py-1 text-xs bg-primary text-primary-content rounded-lg whitespace-nowrap">
                Request Sent
              </span>
            ) : requestStatus === 'pending' ? (
              <span className="px-2 py-1 text-xs bg-base-300 text-base-content rounded-lg whitespace-nowrap">
                Sending...
              </span>
            ) : requestStatus === 'error' ? (
              <span className="px-2 py-1 text-xs bg-error text-error-content rounded-lg whitespace-nowrap">
                Failed
              </span>
            ) : (
              <button
                ...
                className="p-1.5 bg-primary text-primary-content rounded-lg hover:bg-primary/90 disabled:bg-primary/50 transition-colors"
                title="Add friend"
              >
                <UserPlus size={14} />
              </button>
        ...
        <div className="space-y-3 p-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="flex items-center gap-3 animate-pulse">
              <div className="w-10 h-10 bg-base-300 rounded-xl flex-shrink-0" />
    ...
    if (searchQuery) {
      return (
        <div className="text-center py-8 text-base-content/50">
          <Search size={48} className="mx-auto mb-3 text-base-content/30" />
